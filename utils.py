...
def to_psnr(pred_image:Tensor, gt:Tensor):
...
def validation(net:torch.nn.Module, val_data_loader1, device, exp_name):
    print("Start Val")
    psnr_list = []
    ssim_list = []
    for batch_id, test_data in  enumerate(tqdm(val_data_loader1)):
        ...
        # TODO 只循環了一遍
        psnr_list.append(to_psnr(pred_image, gt).item()), ssim_list.append(ssim(pred_image, gt).item())
